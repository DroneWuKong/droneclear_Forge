...
// analytic ranking to appear current.
...
function serveParsed(data, type, source) {
...
function parseAndServe(raw, type, source) {
  try {
    return serveParsed(JSON.parse(raw), type, source);
  } catch (error) {
    return resp(
      {
...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      if (raw) {
        return parseAndServe(raw, type, 'kv');
      }
    } catch (error) {
      console.error(
      ...
      for (const tryPath of [
        ...
        try {
          ...
          if (staticResponse.ok) {
            ...
            return parseAndServe(raw, type, `static:${tryPath}`);
          }
        } catch {
