/**
 * faa-lookup — CF Worker replacing /.netlify/functions/faa-lookup
 * Route: /api/faa-lookup
 *
 * FAA Part 107 Airmen Lookup
 * GET /api/faa-lookup?cert=123456789
 * GET /api/faa-lookup?first=JOHN&last=DOE
 *
 * Env vars (Cloudflare secrets): TURSO_URL, TURSO_AUTH_TOKEN
 */
...
async function tursoQuery(env, sql, args = []) {
  const url = `${env.TURSO_URL}/v2/pipeline`;
  const token = env.TURSO_AUTH_TOKEN;

  if (!env.TURSO_URL || !token) throw new Error('Turso not configured');
